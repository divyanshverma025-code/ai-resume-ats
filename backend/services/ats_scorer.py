...
def _skill_matches(skill: str, text: str, embedder: Any, threshold: float) -> Tuple[bool, float]:
    ...
    # Cheap lexical fallback before computing a vector similarity.
    # This catches spelling/word-order variants without any model memory cost.
    fuzzy_score = fuzz.token_set_ratio(skill, text) / 100.0
    if fuzzy_score >= 0.88:
        return True, fuzzy_score

    # Low-memory deterministic embedding similarity.
